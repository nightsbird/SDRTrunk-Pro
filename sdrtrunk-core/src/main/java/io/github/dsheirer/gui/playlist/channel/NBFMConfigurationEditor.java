...
public class NBFMConfigurationEditor extends ChannelConfigurationEditor
{
    ...
    private VBox createInputGainSection()
    {
        ...
        Label title = new Label("1. Input Gain");
    ...
    {
        VBox section = new VBox(5);
        Label title = new Label("5. Intelligent Squelch");
        title.setFont(Font.font(null, FontWeight.BOLD, 12));
    ...
    private void loadAudioFilterConfiguration(DecodeConfigNBFM config)
    {
        ...
        // Voice Enhancement (stored as noise gate threshold - we repurposed it)
        boolean voiceEnabled = config.isNoiseGateEnabled();
        float voiceAmount = (float)((config.getNoiseGateThreshold() + 40.0) / 20.0) * 50.0f + 30.0f;
        voiceAmount = Math.max(0, Math.min(100, voiceAmount));
        mVoiceEnhanceEnabledSwitch.setSelected(voiceEnabled);
        mVoiceEnhanceSlider.setValue(voiceAmount);
        mVoiceEnhanceLabel.setText((int)voiceAmount + "%");
        mVoiceEnhanceSlider.setDisable(!voiceEnabled);

        // Intelligent Squelch (disabled by default - use existing squelch)
        mSquelchEnabledSwitch.setSelected(false);
        mSquelchThresholdSlider.setValue(-40.0);
        mSquelchThresholdLabel.setText("-40 dB");
        mSquelchReductionSlider.setValue(80.0);
        mSquelchReductionLabel.setText("80%");
    ...
    private void saveAudioFilterConfiguration(DecodeConfigNBFM config)
    {
        ...
        // Voice Enhancement (store as noise gate for compatibility)
        config.setNoiseGateEnabled(mVoiceEnhanceEnabledSwitch.isSelected());
        float voiceAmount = (float)mVoiceEnhanceSlider.getValue();
        float threshold = (voiceAmount - 30.0f) / 50.0f * 20.0f - 40.0f;
        config.setNoiseGateThreshold(threshold);
