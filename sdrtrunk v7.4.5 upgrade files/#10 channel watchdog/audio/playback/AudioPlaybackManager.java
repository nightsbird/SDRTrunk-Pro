...
import io.github.dsheirer.audio.broadcast.AudioActivityWatchdog;
...
public class AudioPlaybackManager implements Listener<AudioSegment>, IAudioController
{
    ...
    private final AudioActivityWatchdog mAudioActivityWatchdog;
    ...
    public AudioPlaybackManager(UserPreferences userPreferences)
    {
        ...
        userPreferences.setAudioActivityWatchdog(mAudioActivityWatchdog);
    ...
    public void receive(AudioSegment audioSegment)
    {
        ...
        // Audio routing happens in processAudioSegments() when audio is ready
    }
    ...
    private void processAudioSegments()
    {
        ...
        while(newSegment != null)
        {
            ...
            {
                if(newSegment.hasBroadcastChannels())
                {
                    mLog.info("Streamable segment detected - routing to: {}", newSegment.getBroadcastChannels());
                }
                mAudioRouter.route(newSegment); // Route to VAC when audio is ready
                mAudioActivityWatchdog.notifyAudioActivity(newSegment);
        ...
        if(!mPendingAudioSegments.isEmpty())
        {
            ...
            while(it.hasNext())
            {
                ...
                else if(audioSegment.hasAudio())
                {
                    ...
                    {
                        mLog.info("Streamable pending segment detected - routing to: {}", audioSegment.getBroadcastChannels());
                    }
                    mAudioRouter.route(audioSegment); // Route to VAC when audio is ready
                    mAudioActivityWatchdog.notifyAudioActivity(audioSegment);
        ...
        if(!mAudioSegments.isEmpty())
        {
            ...
            while(it.hasNext())
            {
				...
				// Route continuously to catch new buffers as they arrive
