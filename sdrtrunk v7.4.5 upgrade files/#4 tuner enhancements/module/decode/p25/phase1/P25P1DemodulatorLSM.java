    ...
    private void processDemodulation(float[] i, float[] q)
    {
        ...
        while(bufferPointer < bufferReload)
        {
            ...
            if(samplePoint < 1)
            {
                ...
                //Adjust sample gain to normalize symbol magnitude toward OBJECTIVE_MAGNITUDE.
                //Uses a smoothed EMA of the target gain with a symmetric time constant so that
                //gain tracks both signal increases and decreases at the same rate. The cap of
                //MAX_SAMPLE_GAIN prevents noise amplification blow-up on very weak/absent signals.
                magnitude = (float)Math.sqrt(iCurrent * iCurrent + qCurrent * qCurrent);

                if(magnitude > 0 && !Float.isInfinite(magnitude))
                {
                    //Target gain for this symbol, capped at MAX_SAMPLE_GAIN
                    requiredGain = Math.min(OBJECTIVE_MAGNITUDE / magnitude, MAX_SAMPLE_GAIN);
                    //Symmetric EMA: same alpha for increases and decreases
                    sampleGain += (requiredGain - sampleGain) * GAIN_ALPHA;
                    //Hard ceiling — never exceed MAX_SAMPLE_GAIN regardless of smoothing overshoot
                    sampleGain = Math.min(sampleGain, MAX_SAMPLE_GAIN);
                }
