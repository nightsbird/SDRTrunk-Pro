        ...
        ISourceEventListener, ISourceEventProvider, Listener<ComplexSamples>
{
    ...
    // -------------------------------------------------------------------------
    // Idle/stuck watchdog — detects a tuner locking up and delivering a frozen
    // or fully-blanked signal without recovering on its own.
    //
    // LSM simulcast nulls can cause extended sync loss and zero-message periods
    // that are NOT indicative of a stuck tuner, so the thresholds here are
    // intentionally longer than the C4FM decoder's equivalent watchdog.
    //
    // Detection requires all three conditions simultaneously:
    //   1. NoiseBlanker blankedRatio > 95%  (near-total blanking)
    //   2. NoiseBlanker avgPower frozen     (< 10% change since last check)
    //   3. Zero valid P25 messages decoded since the last check
    //
    // Stage 1 (soft reset): after WATCHDOG_SOFT_RESET_INTERVALS stuck intervals,
    //   resets noise blanker, IQ corrector, and PLL.
    // Stage 2 (hard reset): if still stuck WATCHDOG_HARD_RESET_INTERVALS intervals
    //   after Stage 1, performs a full decoder reinit via setSampleRate().
    // -------------------------------------------------------------------------
    ...
    private static final int   WATCHDOG_SOFT_RESET_INTERVALS = 8;      // ~8 min — allow time for simulcast nulls
    private static final int   WATCHDOG_HARD_RESET_INTERVALS = 4;      // ~4 min after soft reset
    ...
    {
        // Wrap the message listener so every valid message increments the watchdog counter.
     ...
     * See field declarations above for full description of detection logic and thresholds.
     */
    private void checkWatchdog()
    {
        ...
        if(blankStuck && powerFrozen && noMessages)
        {
            ...
            int threshold = mWatchdogSoftResetDone ? WATCHDOG_HARD_RESET_INTERVALS : WATCHDOG_SOFT_RESET_INTERVALS;
            LOGGER.debug("Watchdog(LSM): stuck [{}/{}] blanked={} powerDelta={}% msgs=0",
                mWatchdogStuckIntervals, threshold,
                mNoiseBlanker.getBlankedPercent(),
                String.format("%.1f", powerDelta * 100.0f));

            if(!mWatchdogSoftResetDone && mWatchdogStuckIntervals >= WATCHDOG_SOFT_RESET_INTERVALS)
            {
                LOGGER.warn("Watchdog(LSM) Stage 1: soft reset after {} stuck intervals — resetting noise blanker, IQ corrector, PLL (blanked={} avgPwr={})",
                    mWatchdogStuckIntervals,
                    mNoiseBlanker.getBlankedPercent(),
            ...
            else if(mWatchdogSoftResetDone && mWatchdogStuckIntervals >= WATCHDOG_HARD_RESET_INTERVALS)
            {
                LOGGER.warn("Watchdog(LSM) Stage 2: hard reset after soft reset failed — reinitialising full decoder pipeline (blanked={} avgPwr={} sampleRate={})",
                    mNoiseBlanker.getBlankedPercent(),
                    String.format("%.2e", currentAvgPower),
                    mCurrentSampleRate);
        ...
        else
        {
            ...
            {
                LOGGER.debug("Watchdog(LSM): cleared after {} stuck intervals (blanked={} powerDelta={}% msgs={})",
                    mWatchdogStuckIntervals,
                    mNoiseBlanker.getBlankedPercent(),
                    String.format("%.1f", powerDelta * 100.0f),
                    msgCount);
