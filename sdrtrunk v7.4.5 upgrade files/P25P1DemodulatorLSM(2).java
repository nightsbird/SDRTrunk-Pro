        ...
        while(bufferPointer < bufferReload)
        {
            ...
            if(samplePoint < 1)
            {
                ...
                if(magnitude > 0 && !Float.isInfinite(magnitude))
                {
                    ...
                    // Slew-rate limited AGC: caps gain increase to 20% of current gain per symbol.
                    // Prevents the original Math.min snap from instantly jumping to requiredGain
                    // during a simulcast fade/null, which amplified noise into the TED and PLL.
                    // Decay (gain reduction) remains at the original 5% rate for smooth tracking.
                    float gainError = requiredGain - sampleGain;
                    float maxGainIncrease = sampleGain * 0.20f;
                    if(gainError > maxGainIncrease)
                    {
                        sampleGain += maxGainIncrease;
                    }
                    else
                    {
                        sampleGain += gainError * 0.05f;
                    }
                ...
                //Gate on post-AGC magnitude to detect true carrier absence vs weak-but-present signal.
                //Post-AGC magnitude ≈ OBJECTIVE_MAGNITUDE (1.0) when signal present, near zero when absent.
                //Coasting (timingAdjustment = 0) through a true null holds timing at its last good value,
                //preventing the drift that causes sync loss bursts on recovery.
                timingAdjustment = 0;
                float postMagnitude = magnitude * sampleGain;
                if(postMagnitude > OBJECTIVE_MAGNITUDE * 0.15f)
                {
