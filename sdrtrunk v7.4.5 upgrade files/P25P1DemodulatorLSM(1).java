        ...
        while(bufferPointer < bufferReload)
        {
            ...
            if(samplePoint < 1)
            {
                ...
                if(magnitude > 0 && !Float.isInfinite(magnitude))
                {
                    ...
                    // Slew-rate limited AGC: max 10% of current gain change per symbol.
                    // Removes the Math.min snap that previously allowed gain to jump
                    // instantly to requiredGain during a simulcast fade, amplifying noise
                    // and destabilizing both the Gardner TED and PLL through the null.
                    float maxGainStep = sampleGain * 0.10f;
                    float gainError = requiredGain - sampleGain;
                    sampleGain += constrain(gainError * 0.05f, maxGainStep);
                ...
                //Coast (freeze timing) during low-signal conditions: when magnitude is below
                //10% of the objective, the TED output is dominated by noise. Holding the last
                //good timing offset through a simulcast null prevents drift that causes sync
                //loss on recovery. Threshold chosen to be well above thermal noise floor but
                //below any valid signal level after AGC.
                timingAdjustment = 0;
                if(magnitude > OBJECTIVE_MAGNITUDE * 0.10f)
                {
