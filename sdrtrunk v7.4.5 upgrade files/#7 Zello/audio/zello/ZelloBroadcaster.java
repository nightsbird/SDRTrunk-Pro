    ...
    implements IRealTimeAudioBroadcaster
{
    ...
    // Opus packets encoded before the stream_id response arrives are held here
    // and flushed as soon as the stream_id is known.
    private final LinkedTransferQueue<byte[]> mPendingPackets = new LinkedTransferQueue<>();
    private final AtomicBoolean mStopPendingStreamId = new AtomicBoolean(false);
    ...
    // Guards mResampleBuffer, mResampleBufferPos, mPreviousSample, and mOpusEncoder
    // against concurrent access between the scheduled encoder task and stopRealTimeStream().
    ...
    public void start()
    {
        // Always clear mStopped on start — even if we skip reconnecting below
        mStopped.set(false);

        // If already connected and channel is online, don't reinitialize.
        // The channel manager calls stop()/start() during transient restarts and
        // stop() preserves the connection when still connected. Nothing to do here.
        if(mConnected.get() && mChannelOnline.get())
        {
            mLog.debug("{}Zello start() called but already connected and online, skipping", ch());
            return;
        }

        // If connected but not yet online (on_channel_status not yet received), also skip —
        // the channel status message will arrive shortly and set mChannelOnline.
        if(mConnected.get())
        {
            mLog.debug("{}Zello start() called but WebSocket already open, waiting for channel status", ch());
            return;
        }
    ...
    public void stop()
    {
        ...
        mStopPendingStreamId.set(false);

        // If the WebSocket is still alive and connected, preserve the connection.
        // The channel manager calls stop()/start() during transient restarts (e.g. carrier
        // offset corrections). Disconnecting here would clear mChannelOnline and cause
        // isRealTimeReady() to return false for several seconds while the server sends a
        // new on_channel_status, dropping real voice transmissions.
        // start() already skips reconnecting when mConnected is true, so the connection
        // will survive the cycle. Only truly disconnect on dispose().
        if(!mConnected.get())
        {
            disconnectWebSocket();
        ...
        {
            mLog.debug("{}Zello stop() called while connected — preserving connection for restart", ch());
        }
    }
    ...
    public void stopRealTimeStream()
    {
        ...
        // Cancel the encoder task and wait for any in-progress run to finish before
        // we touch the shared audio state below. cancel(false) is NOT sufficient — the
        // scheduled task may already be executing and will race with us on mResampleBuffer.
        if(mEncoderFuture != null) { mEncoderFuture.cancel(false); mEncoderFuture = null; }

        // Hold the audio lock while draining the queue and flushing the resample buffer
        // so we cannot race with a final encoder-task execution that slipped through.
        ...
        if(streamId > 0)
        {
            // Flush any packets that were queued before stream_id arrived (shouldn't normally
            // happen here, but defensive in case of very short transmissions)
            flushPendingPackets(streamId);
        ...
        {
            // stream_id hasn't arrived yet — set flag so the stream_id handler will
            // send stop_stream as soon as it receives the ID.
            mStopPendingStreamId.set(true);
            mLog.debug("{}stopRealTimeStream called before stream_id received — deferring stop_stream", ch());
        }
    ...
    {
        long streamId = mCurrentStreamId.get();
        // streamId == -2 means start_stream was rejected; drop the frame
        ...
        try
        {
            ...
            if(encoded > 0)
            {
                ...
                if(streamId > 0)
                {
                    // Flush any packets queued before stream_id arrived, then send this one
                    flushPendingPackets(streamId);
                    sendAudioPacket(streamId, opusFrame);
                }
                else
                {
                    // stream_id not yet received — hold packet until it arrives
                    mPendingPackets.offer(opusFrame);
                }
            }
        }
        catch(Exception e)
        {
            mLog.error("Opus encoding error - reinitializing encoder", e);
            // The Concentus encoder's internal resampler can enter a bad state after certain
            // failures (AssertionError in silk_resampler). Reset position and encoder to
            // prevent ArrayIndexOutOfBoundsException on subsequent frames.
            mResampleBufferPos = 0;
            mPendingPackets.clear();
            try { initOpusEncoder(); }
            catch(Exception ex) { mLog.error("Failed to reinitialize Opus encoder", ex); }
        }
    }

    /** Send all packets queued before stream_id was known. */
    private void flushPendingPackets(long streamId)
    {
        byte[] pending;
        while((pending = mPendingPackets.poll()) != null)
        {
    ...
    {
        if(mResampleBufferPos <= 0) return; // Nothing to flush
    ...
    private void connectWebSocket()
    {
        ...
        // Don't reconnect if we've been deliberately stopped
        if(mStopped.get())
        {
            mReconnecting.set(false);
            return;
    ...
    private void scheduleReconnect()
    {
        if(mKicked.get())
        {
            ...
            long backoff = KICKED_BACKOFF_MS * (1L << Math.min(kickCount - 1, 4)); // exponential: 60s, 120s, 240s, 480s, 960s
            mLog.warn("{}Zello kicked - backing off {}s ({}/{})", ch(), backoff / 1000, kickCount, MAX_KICKED_RETRIES);
    ...
    private class ZelloWebSocketListener implements WebSocket.Listener
    {
        ...
        public CompletionStage<?> onClose(WebSocket ws, int code, String reason)
        {
            ...
            // Don't reconnect if we're intentionally stopped or if kicked handler
            // already scheduled the reconnect.
            ...
            try
            {
                ...
                if(json.has("command"))
                {
                    ...
                    if("on_channel_status".equals(command))
                    {
                        ...
                        {
                            // If on_channel_status arrives before the logon success response
                            // (a known Zello server race at startup), the channel is clearly
                            // authenticated — set mConnected here too so isRealTimeReady()
                            // returns true without requiring a disable/re-enable cycle.
                            if(!mConnected.get())
                            {
                                mLog.debug("{}on_channel_status online received before logon ack — setting connected", ch());
                                mConnected.set(true);
                                mKicked.set(false);
                            }
                            // getAndSet returns old value; only log/set state on first transition
                ...
                if(json.has("stream_id") && json.has("success"))
                {
                    if(json.get("success").getAsBoolean())
                    {
                        ...
                        // Flush any Opus packets that were encoded before stream_id arrived
                        flushPendingPackets(streamId);

                        // If stopRealTimeStream() was called before stream_id arrived,
                        // send the deferred stop_stream now so the stream closes cleanly.
                        if(mStopPendingStreamId.getAndSet(false))
                        {
                            mLog.debug("{}Sending deferred stop_stream for stream_id={}", ch(), streamId);
                            sendStopStream(streamId);
                            mStreamedCount.incrementAndGet();
                            mKickedCount.set(0);
                            broadcast(new BroadcastEvent(ZelloBroadcaster.this,
