...
public class NBFMConfigurationEditor extends ChannelConfigurationEditor
{
    ...
    private ComboBox<String> mSquelchModeCombo;
    private Slider mActivationTimeSlider;
    private Label mActivationTimeLabel;
    ...
    {
        VBox section = new VBox(5);
        Label title = new Label("4. Intelligent Squelch");
        title.setFont(Font.font(null, FontWeight.BOLD, 12));

        mSquelchEnabledSwitch = new ToggleSwitch("Enable Intelligent Squelch");
        mSquelchEnabledSwitch.setTooltip(new Tooltip("Detect and gate out FM carrier noise"));
        mSquelchEnabledSwitch.selectedProperty().addListener((obs, old, val) -> {
            if(!mLoadingConfiguration)
            {
                ...
                mSquelchModeCombo.setDisable(!val);
                mActivationTimeSlider.setDisable(!val);
        ...
        mSquelchThresholdSlider = new Slider(-60, -20, -40);
        mSquelchThresholdSlider.setMajorTickUnit(10);
        ...
        mSquelchThresholdSlider.setTooltip(new Tooltip("Detection sensitivity\nDefault: -40 dB"));
        mSquelchThresholdSlider.valueProperty().addListener((obs, old, val) -> {
            if(!mLoadingConfiguration)
            {
                mSquelchThresholdLabel.setText(val.intValue() + " dB");
                modifiedProperty().set(true);
            }
        });
        ...
        mSquelchThresholdLabel = new Label("-40 dB");
        ...
        mSquelchReductionSlider.setTooltip(new Tooltip("Amount to reduce carrier noise\nDefault: 80%"));
        ...
        Label modeLabel = new Label("Detection Mode:");
        GridPane.setConstraints(modeLabel, 0, 2);
        controlsPane.getChildren().add(modeLabel);

        mSquelchModeCombo = new ComboBox<>();
        mSquelchModeCombo.getItems().addAll("Energy Only", "Spectral Only", "Hybrid (Recommended)");
        mSquelchModeCombo.setValue("Hybrid (Recommended)");
        mSquelchModeCombo.setTooltip(new Tooltip("Energy: Simple RMS threshold\nSpectral: Frequency analysis\nHybrid: Both (best for most cases)"));
        mSquelchModeCombo.setOnAction(e -> {
            if(!mLoadingConfiguration) modifiedProperty().set(true);
        });
        GridPane.setConstraints(mSquelchModeCombo, 1, 2);
        controlsPane.getChildren().add(mSquelchModeCombo);

        Label activationLabel = new Label("Activation Time:");
        GridPane.setConstraints(activationLabel, 0, 3);
        controlsPane.getChildren().add(activationLabel);

        mActivationTimeSlider = new Slider(0, 500, 100);
        mActivationTimeSlider.setMajorTickUnit(100);
        mActivationTimeSlider.setMinorTickCount(4);
        mActivationTimeSlider.setShowTickMarks(true);
        mActivationTimeSlider.setShowTickLabels(true);
        mActivationTimeSlider.setPrefWidth(300);
        mActivationTimeSlider.setTooltip(new Tooltip("Minimum voice duration to open gate\nPrevents noise bursts\nDefault: 100ms"));
        mActivationTimeSlider.valueProperty().addListener((obs, old, val) -> {
            if(!mLoadingConfiguration)
            {
                mActivationTimeLabel.setText(val.intValue() + " ms");
                modifiedProperty().set(true);
            }
        });
        GridPane.setConstraints(mActivationTimeSlider, 1, 3);
        controlsPane.getChildren().add(mActivationTimeSlider);

        mActivationTimeLabel = new Label("100 ms");
        GridPane.setConstraints(mActivationTimeLabel, 2, 3);
        controlsPane.getChildren().add(mActivationTimeLabel);

        Label holdLabel = new Label("Hold Time:");
        GridPane.setConstraints(holdLabel, 0, 4);
        controlsPane.getChildren().add(holdLabel);

        mHoldTimeSlider = new Slider(0, 1000, 200);
        mHoldTimeSlider.setMajorTickUnit(200);
        ...
        GridPane.setConstraints(mHoldTimeSlider, 1, 4);
        controlsPane.getChildren().add(mHoldTimeSlider);

        mHoldTimeLabel = new Label("200 ms");
        GridPane.setConstraints(mHoldTimeLabel, 2, 4);
    ...
    private void loadAudioFilterConfiguration(DecodeConfigNBFM config)
    {
        ...
        mSquelchThresholdLabel.setText((int)config.getNoiseGateThreshold() + " dB");
        ...
        // Load squelch mode
        String mode = config.getNoiseGateMode();
        if (mode.equals("ENERGY_ONLY")) {
            mSquelchModeCombo.setValue("Energy Only");
        } else if (mode.equals("SPECTRAL_ONLY")) {
            mSquelchModeCombo.setValue("Spectral Only");
        } else {
            mSquelchModeCombo.setValue("Hybrid (Recommended)");
        }
        
        // Load activation time
        int activationTime = config.getNoiseGateActivationTime();
        mActivationTimeSlider.setValue(activationTime);
        mActivationTimeLabel.setText(activationTime + " ms");
        ...
        mSquelchModeCombo.setDisable(!squelchEnabled);
        mActivationTimeSlider.setDisable(!squelchEnabled);
    ...
    private void saveAudioFilterConfiguration(DecodeConfigNBFM config)
    {
        ...
        // Save squelch mode
        String modeValue = mSquelchModeCombo.getValue();
        if (modeValue.equals("Energy Only")) {
            config.setNoiseGateMode("ENERGY_ONLY");
        } else if (modeValue.equals("Spectral Only")) {
            config.setNoiseGateMode("SPECTRAL_ONLY");
        } else {
            config.setNoiseGateMode("HYBRID");
        }
        
        // Save activation time
        config.setNoiseGateActivationTime((int)mActivationTimeSlider.getValue());
