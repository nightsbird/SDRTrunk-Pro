...
public class DecodeConfigNBFM extends DecodeConfigAnalog
{
    ...
    private float mNoiseGateThreshold = -40.0f; // dB
    private float mNoiseGateReduction = 0.8f; // 0.0 to 1.0
    private String mNoiseGateMode = "HYBRID"; // ENERGY_ONLY, SPECTRAL_ONLY, HYBRID
    private int mNoiseGateActivationTime = 100; // milliseconds
    private int mNoiseGateHoldTime = 200; // milliseconds
    ...
    /**
     * Gets the noise gate threshold in dB.
     * @return threshold (default -40 dB)
     */
     ...
     * @param threshold in dB (recommended -40 to -50 dB)
     */
    ...
    /**
     * Gets the noise gate detection mode.
     * @return mode string: ENERGY_ONLY, SPECTRAL_ONLY, or HYBRID
     */
    @JacksonXmlProperty(isAttribute = true, localName = "noiseGateMode")
    public String getNoiseGateMode()
    {
        return mNoiseGateMode;
    }

    /**
     * Sets the noise gate detection mode.
     * @param mode ENERGY_ONLY, SPECTRAL_ONLY, or HYBRID
     */
    public void setNoiseGateMode(String mode)
    {
        mNoiseGateMode = mode;
    }

    /**
     * Gets the noise gate activation time threshold in milliseconds.
     * @return activation time (default 100ms)
     */
    @JacksonXmlProperty(isAttribute = true, localName = "noiseGateActivationTime")
    public int getNoiseGateActivationTime()
    {
        return mNoiseGateActivationTime;
    }

    /**
     * Sets the noise gate activation time threshold.
     * @param timeMs Minimum duration voice must be present to open gate (0-500ms)
     */
    public void setNoiseGateActivationTime(int timeMs)
    {
        mNoiseGateActivationTime = Math.max(0, Math.min(500, timeMs));
    }

    /**
     * Gets the noise gate hold time in milliseconds.
     * @return hold time (default 200ms)
     */
