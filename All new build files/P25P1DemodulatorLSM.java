...
public class P25P1DemodulatorLSM
{
    ...
    // PLL acquisition vs. tracking mode: mSyncCount is reset to a threshold on each sync detect
    // and decremented each symbol. When > 0, the loop is in "locked/tracking" mode with a
    // reduced PLL gain. When it reaches 0, the loop switches to "acquiring" mode with the
    // original high gain to re-lock quickly after sync loss.
    private int mSyncCount = 0;
    private static final int SYNC_COUNT_THRESHOLD = 192; // ~40ms at 4800 sym/sec
    ...
    {
        mPLL = 0f;
        mSyncCount = 0; // Force acquisition mode after frequency/PPM change
    }
    ...
    public void process(float[] i, float[] q)
    {
        ...
        // PLL gain: high (0.1) during acquisition for fast carrier lock, low (0.04) when locked for
        // stable noise-resistant tracking. Switched via mSyncCount — see resetPLL() and sync path below.
        // Original constant 0.1 caused noise-chasing on locked channels; 0.04 alone caused slow initial lock.
        // tedGain and phaseError clamp are also mode-dependent: use original values during acquisition
        // so that timing and carrier both converge fast enough to find the first sync+NID.
        double tedGain = (syncCount > 0) ? samplesPerSymbol / 6.0 : samplesPerSymbol / 4.0;
        ...
        while(bufferPointer < bufferReload)
        {
            ...
            if(samplePoint < 1)
            {
                ...
                if(magnitude > 0 && !Float.isInfinite(magnitude))
                {
                    ...
                    // Dual-rate AGC: fast attack for large errors (fading recovery), slow decay for stability
                    float gainError = requiredGain - sampleGain;
                    float gainRate = Math.abs(gainError) > 0.5f ? 0.15f : 0.05f;
                    sampleGain += gainError * gainRate;
                ...
                // Decay sync counter each symbol. When > 0 = locked/tracking mode; 0 = acquiring.
                if(syncCount > 0)
                {
                    syncCount--;
                }
                ...
                if(softSymbol != 0)
                {
                    ...
                    // Phase error clamp: wide (0.3) during acquisition for fast convergence,
                    // tight (0.18) when locked to reduce noise injection into the PLL.
                    float phaseClamp = (syncCount > 0) ? 0.18f : 0.3f;
                    phaseError = constrain(softSymbol - hardSymbol.getIdealPhase(), phaseClamp);
                    // Dual-mode PLL gain: fast acquisition when unlocked (0.1), slow stable tracking when locked (0.04).
                    float pllGain = (syncCount > 0) ? 0.04f : 0.1f;
                ...
                if(mMessageFramer.processWithSoftSyncDetect(softSymbol, hardSymbol))
                {
                    ...
                    // Sync+NID validated — enter/maintain tracking mode with reduced PLL gain.
                    syncCount = SYNC_COUNT_THRESHOLD;
                }
