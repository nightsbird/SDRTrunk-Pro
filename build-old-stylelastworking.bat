@echo off
echo ========================================
echo Building SDRTrunk - Old Structure Style
echo bin/lib format like v0.6.1
echo ========================================
echo.
...
jlink --module-path "%JAVA_HOME%\jmods;C:\Java\javafx-jmods-22" ^
...
echo Step 3: Creating distribution structure (old style)...
...
cd build
if exist sdr-trunk-windows-x86_64-v0.6.2.zip del sdr-trunk-windows-x86_64-v0.6.2.zip
powershell -command "Compress-Archive -Path sdr-trunk-package -DestinationPath sdr-trunk-windows-x86_64-v0.6.2.zip -Force"
cd ..

echo ========================================
echo SUCCESS! Package created:
echo build\sdr-trunk-windows-x86_64-v0.6.2.zip
echo.
echo Structure matches v0.6.1:
echo - bin\sdr-trunk.bat (launcher)
echo - bin\java.exe (and all JRE binaries)
echo - lib\*.jar (all application JARs)
echo - conf\ (JRE configuration)
echo - legal\ (licenses)
echo.
echo Extract and run: bin\sdr-trunk.bat
echo ========================================
goto :end
