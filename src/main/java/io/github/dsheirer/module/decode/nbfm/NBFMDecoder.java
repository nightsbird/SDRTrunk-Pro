        ...
        Listener<ComplexSamples>, IRealBufferProvider, IDecoderStateEventProvider, INoiseSquelchController
{
    ...
    public NBFMDecoder(DecodeConfigNBFM config)
    {
        ...
        //Initialize audio filters - will be configured when sample rate is set
        mAudioFilters = new NBFMAudioFilters(DEMODULATED_AUDIO_SAMPLE_RATE);
        updateAudioFilterConfiguration();

        //Send squelch controlled audio to the audio filters, then resampler, and notify the decoder state that the call continues.
        mNoiseSquelch.setAudioListener(audio -> {
            //Apply audio filters before resampling
    ...
    /**
     * Update audio filters from configuration
     */
    private void updateAudioFilterConfiguration()
    {
        if(mAudioFilters != null)
        {
            DecodeConfigNBFM config = getDecodeConfiguration();

            // De-emphasis
            mAudioFilters.setDeemphasisEnabled(config.isDeemphasisEnabled());
            mAudioFilters.setDeemphasisTimeConstant(config.getDeemphasisTimeConstant());

            // Low-pass
            mAudioFilters.setLowPassEnabled(config.isLowPassEnabled());
            mAudioFilters.setLowPassCutoff(config.getLowPassCutoff());

            // Noise gate
            mAudioFilters.setNoiseGateEnabled(config.isNoiseGateEnabled());
            mAudioFilters.setNoiseGateThreshold(config.getNoiseGateThreshold());
            mAudioFilters.setNoiseGateReduction(config.getNoiseGateReduction());

            // AGC
            mAudioFilters.setAgcEnabled(config.isAgcEnabled());
            mAudioFilters.setAgcParameters(config.getAgcTargetLevel(), config.getAgcMaxGain());
        }
    }
    ...
    private void setSampleRate(double sampleRate)
    {
        ...
        if(coefficients == null)
        {
            mLog.info("Unable to use remez filter designer for sample rate [" + decimatedSampleRate + 
                    "] pass band stop [" + passBandStop + "] and stop band start [" + stopBandStart + 
                    "] - will proceed using simple low pass filter design");
